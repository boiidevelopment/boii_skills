...
CREATE TABLE IF NOT EXISTS `characters_base_skills` (
    ...
    KEY (`id`),
    CONSTRAINT `fk_characters_base_skills_users` FOREIGN KEY (`identifier`)
    REFERENCES `users` (`identifier`) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS `characters_civ_skills` (
    ...
    PRIMARY KEY (`identifier`),
    KEY (`id`),
    CONSTRAINT `fk_characters_civ_skills_users` FOREIGN KEY (`identifier`)
    REFERENCES `users` (`identifier`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1;
...
CREATE TABLE IF NOT EXISTS `characters_crim_skills` (
    ...
    PRIMARY KEY (`identifier`),
    KEY (`id`),
    CONSTRAINT `fk_characters_crim_skills_users` FOREIGN KEY (`identifier`)
    REFERENCES `users` (`identifier`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1;